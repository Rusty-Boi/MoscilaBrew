<nav class="navbar fixed-top navbar-expand-lg bg-body-tertiary">
    <div class="container-fluid">
      <a class="navbar-brand mr-20" href="/">MoscilaBrew</a>
      <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
      </button>
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav nav-underline me-auto mb-2 mb-lg-0 m-auto">
            <a class="nav-link {{request() -> is('/') ? 'disabled' : ''}}" href="/">Home</a>
            <a class="nav-link {{request() -> is('catalog') ? 'disabled' : ''}}" href="/catalog">Catalog</a>
            <a class="nav-link {{request() -> is('featured') ? 'disabled' : ''}}" href="/featured">Featured</a>
            <a class="nav-link {{request() -> is('aboutUs') ? 'disabled' : ''}}" href="/aboutUs">About Us</a>
        </ul>
